...
WEATHER_API_KEY = os.getenv('OpenWeatherMap')
...
async def warn(ctx, member: discord.Member = None, *, reason: str = "Không có lý do"):
    if member is None: return await ctx.send("❌ Cú pháp: `!warn @user <lý do>`")
    ...
    save_json(data)
    await ctx.send(f"⚠️ **Cảnh cáo:** {member.mention}\n**Lý do:** {reason}\n**Tổng số:** {len(data[uid])} lần.")
...
async def warns(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(title=f"Lịch sử cảnh cáo: {member.display_name}", color=discord.Color.orange())
    for i, item in enumerate(user_warns, 1):
        embed.add_field(name=f"Lần {i}", value=f"Lý do: {item['reason']}\n*Ngày: {item['time']}*", inline=False)
...
# ==========================================
# 7. THÔNG TIN & TIỆN ÍCH (USERINFO, AVATAR, POLL)
# ==========================================
...
async def serverinfo(ctx):
    try:
        g = ctx.guild
        # Kiểm tra nếu g.owner bị None (do chưa load kịp hoặc thiếu intent)
        owner_mention = g.owner.mention if g.owner else f"ID: {g.owner_id}"

        embed = discord.Embed(title=f"🛡️ Thông tin Server: {g.name}", color=discord.Color.blue())
...
async def weather(ctx, *, city: str):
    # Link API của OpenWeatherMap
    base_url = "http://api.openweathermap.org/data/2.5/weather?"

    # Tham số: q là tên thành phố, appid là key, units=metric để lấy độ C
    complete_url = f"{base_url}q={city}&appid={WEATHER_API_KEY}&units=metric&lang=vi"
    ...
    try:
        ...
        if data["cod"] != "404":
            # Trích xuất thông tin từ JSON trả về
            main = data["main"]
            ...
            # Tạo Embed cho đẹp
            embed = discord.Embed(
                title=f"Thanh phố: {city_name}, {country}",
                description=f"Thông tin thời tiết hiện tại",
                color=discord.Color.blue()
            )
            embed.add_field(name="Nhiệt độ", value=f"{temp}°C", inline=True)
            embed.add_field(name="Độ ẩm", value=f"{humidity}%", inline=True)
            embed.add_field(name="Trạng thái", value=description.capitalize(), inline=False)
        ...
        await ctx.send("⚠️ Có lỗi xảy ra khi kết nối với API.")
        print(e)
...
COMMAND_LIST = [
    ...
    {
        "title": "THÔNG TIN & TIỆN ÍCH (USERINFO, AVATAR, POLL, ...)",
        "commands":[
            ...
            ("!weather", None, "!weather", "Lấ thông tin thời tiết tại London."),
        ]
    }
]
...
async def help(ctx):
    ...
    for category in COMMAND_LIST:
        ...
        cmd_text = ""
        # FIX: Khai báo đủ 4 biến cmd, slash, syntax, desc để khớp với danh sách
...
async def slash_help(interaction: discord.Interaction):
    ...
    for category in COMMAND_LIST:
        field_title = category["title"]
        cmd_text = ""
        # FIX: Khai báo đủ 4 biến ở đây để tránh lỗi ValueError
        for cmd, slash, syntax, desc in category["commands"]:
            cmd_text += f"**{syntax}**\n└ *{desc}* (Slash: `{slash}`)\n"
